...
require('em.js')('sparkflow',function(em){
  ...
  mixins.StreamMixin = function(){
      ...
      var es = streams.newEventStream('socketStream'),
          ...
          grouped =  streams.combineUnOrderByStreams(function(streams,recieved){
              ...
              if(!asSingleStream) return this.stream.massEmit(received);
              return this.stream.emit(received.join(delimiter));
          })(bg,data,eg);
          ...
          bg.initd.add(function(n){
            if(!ready.ready()) data.resume();
            ready.switchState('lock');
            data.pause();
          });

          eg.initd.add(function(n){
            data.resume();
            ready.switchState('unlock');
          });

          grouped.initd.add(function(){
        ...
        return {
          ...
          enableDelimitedGroupStream: function(delimited){
             if(!delimited) delimited = '/';
             asSingleStream = true;
             delimiter = delimited;
          },
          disableDelimitedGroupStream: function(){
             asSingleStream = true;
             delimiter = '';
          }
        };
  ...
  mixins.SocketMixin = function(){
      ...
      return {
        ...
        totalConnected: function(){
          return socketSubscribers.length();
        },
        ...
        disconnected: function(){
          return valids.notExist(to);
        },
        connected: function(){
          return valids.exists(to);
        },
  ...
  mixins.PortMixin = function(){
    ...
    delimitedEnabled = false,delimiter = '',
    ...
    return {
      ...
      createSocket: function(alias){
          ...
          alias = (valids.exists(alias) ? alias : id);
          aliases.add(alias,id);
          if(this.isDelimiterEnabledForAllSockets()) newSocks.enabledelimitedGroupStream(delimiter);
          return newSocks;
      },
      connect: function(Port,socket,alias){
        ...
        if(this.allowsMultipleSockets()){
          ...
          var available = filter(false),newSocks;
      ...
      leechSocket: function(Port,socket,socketId){
        ...
        if(!valids.exists(socks)) socks = this.socket();
      ...
      beginGroup: function(data,socketID){
        if(socketID){
      ...
      isDelimiterEnabledForAllSockets: function(){
        return !!delimitedEnabled;
      },
      enableDelimiterForSocket: function(dm,alias){
        this.socket(alias).enableDelimitedGroupStream(dm);
      },
      disableDelimiterForSocket: function(alias){
        this.socket(alias).disableDelimitedGroupStream();
      },
      enableDelimiterForAllSockets: function(dm){
        delimitedEnabled = true;
        delimiter = dm;
        sockets.cascade(function(e){
          if(e) e.enableDelimitedGroupStream(dm);
        });
      },
      disableDelimiterForAllSockets: function(){
        delimitedEnabled = false;
        delimiter = '';
        sockets.cascade(function(e){
          if(e) e.disableDelimitedGroupStream();
        });
      },
      ...
      reAlias: function(socket,alias){
        if(!socket || !alias) return;
        var ind = sockets.has(socket);
        return valids.exists(ind) ? alias.add(alias,ind) : null;
      },
      alias: function(alias){
        var ind = aliases.get(alias);
        return valids.exists(ind) ? sockets.nth(ind) : null;
      },
      ID: function(){
      ...
      totalSockets: function(){
        return sockets.length();
      },
      totalConnectedOnSocket: function(alias){
        var sock = this.alias(alias);
        return !!sock && sock.totalConnected();
      },
      close: function(){
        sockets.cascade(function(e){
          e.close();
  ...
  mixins.ComponentMixin = function(){
    ...
    return {
      ...
      connect: function(myPort,component,toPort,mySocketId,toSocketId){
        ...
        return extport.connect(inport,insocket,toSocketId);
      ...
      disconnect: function(myPort,component,toPort,mySocketId,toSocketId){
        ...
        return extport.disconnect(inport,insocket,toSocketId);
      },
      ID: function(){
        return  this.id;
      },
