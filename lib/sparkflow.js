...
require('em.js')('sparkflow',function(em){
  ...
  mixins.PortMixin = function(){
    var finder,filter,sockets = as.AppStack.ArrayDecorator([]);
    ...
    var tagName = function(name){
      return name+':'+sockets.length();
    }

    return {
      init: function(){
         sockets.add(sf.Socket(this.id,this));
         sockets.limit(1);
      },
      sockets: function(){
      ...
      connect: function(Port,socket){
        if(this.allowsMultipleSockets()){
          ...
          newSocks = sf.Socket(tagName(this.id),this);
      ...
      leechSocket: function(Port,socket,socketId){
        if(!Port.ownSocket(socket)) return;
      ...
      socket: function(n){
        ...
        return (!!n && valids.isNumber(n) && sockets.nth(n));
      },
  ...
  mixins.ComponentMixin = function(){
    ...
    return {
      init: function(){
         ...
         this.subgraph = sf.Network(this.name + ':SubgraphNetwork');
      ...
      connect: function(component,toPort,fromId,fromSocketId,toSocketId){
        if(itr.find(component)) return;

        var inport = this.port(toPort),
        insocket = inport ? inport.socket(toSocketId) : null,
        extport = component.port(fromId), 
        extsocket = port ? port.socket(socketId) : null;
        
        if(!inport || !extport) return;
        
        if(insocket && extport) inport.connectSocketToSocket(insocket,extsocket);

        inport.connect(extport,extsocket);
      ...
      disconnect: function(component,toPort,fromId,fromSocketId,toSocketId){
        ...
        if(insocket && extport) inport.disconnectSocketToSocket(insocket,extsocket);
        
        inport.disconnect(extport,extsocket,toSocketId);
      },
      connectedTo: function(component){
        if(!component || !(component instanceof types.Component)) return;
        var find =  lists.find(Component);
        if(find && find.data) return true;
        return false;
      },
      ID: function(){
        return  this.id;
  ...
  sf.Component = inv.MixinCreator(types.Port,mixins.ComponentMixin);
